...
import numpy as np
import torch
from data.dataset import Cityscapes, CityscapesPanoptic, COCOPanoptic
from data.distributed_sampler import InferenceSampler, TrainingSampler
...
def build_dataset_from_cfg(config, is_train=True):
    ...
    dataset_cfg = {
        'cityscapes':
        dict(root=config.DATASET.ROOT,
             split=config.DATASET.TRAIN_SPLIT
             if is_train else config.DATASET.TEST_SPLIT,
             is_train=is_train,
             crop_size=config.DATASET.CROP_SIZE
             if is_train else config.TEST.CROP_SIZE,
