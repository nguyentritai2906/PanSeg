...
from ..transforms import (PanopticTargetGenerator, Resize,
                          SemanticTargetGenerator, build_transforms)
...
# Add 1 void label.
_CITYSCAPES_PANOPTIC_TRAIN_ID_TO_EVAL_ID = [
...
class CityscapesPanoptic(Cityscapes):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(CityscapesPanoptic,
              self).__init__(root, split, is_train, crop_size, mirror,
                             min_scale, max_scale, resize, scale_step_size,
                             mean, std)
        ...
        if split == 'test':
            ...
            json_filename = os.path.join(
                self.root, 'gtFine',
                'cityscapes_panoptic_{}_trainId.json'.format(self.split))
        ...
        self.raw_label_transform = SemanticTargetGenerator(
            self.ignore_label, self.rgb2id)
