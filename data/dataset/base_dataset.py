...
class BaseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        assert os.path.exists(
            self.img_list[index]), 'Path does not exist: {}'.format(
                self.img_list[index])
        ...
        if self.ann_list is not None:
            assert os.path.exists(
                self.ann_list[index]), 'Path does not exist: {}'.format(
                    self.ann_list[index])
        ...
        if self.raw_label_transform is not None:
            raw_label = self.raw_label_transform(
                raw_label, self.ins_list[index])['semantic']
        if not self.is_train:
            # Do not save this during training
